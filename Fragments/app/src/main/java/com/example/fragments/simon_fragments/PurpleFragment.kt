...
class PurpleFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        val view = inflater.inflate(R.layout.fragment_purple, container, false)
        val bg = view.findViewById<TextView>(R.id.purple)
        var original = true
        bg.setOnClickListener {
            val finalColor = R.color.plum
            val originalColor = R.color.purple;
            bg.setBackgroundColor(ContextCompat.getColor(requireContext(), finalColor))

            fun playSound() {
                val resId = resources.getIdentifier("f", "raw", activity?.packageName)
